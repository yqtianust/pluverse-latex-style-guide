...
\usepackage{balance}
\usepackage{enumitem}
% The following packages cause a formatting issue in the ACM template (i.e., the wrong title font size).
...
\usepackage{multicol}
\usepackage{multirow}
% The natbib package conflicts with the cite package in the IEEE template.
% The ACM template handles citations automatically and does not recommend loading natbib explicitly.
% \usepackage{natbib}
...
\usepackage{colortbl}
% For consistent, standard typesetting of numbers and units (e.g., \SI{99.6}{\percent})
\usepackage{siunitx}
\sisetup{group-separator = {,}}
\newcommand{\mypercent}[1]{\SI{#1}{\percent}}
\newcommand{\mypercentrange}[2]{\SIrange{#1}{#2}{\percent}}

\lstset{
  language=C,
  tabsize=1,
  % ==== Line Number ====
  % numbers=left/right/none,  % activate per listing
  % stepnumber=1/2/5/10,
  numberstyle=\tiny\color{gray},
  numbersep=4pt,
  %
  % ==== Using LaTeX Commands Inside ====
  % If you want to use LaTeX commands inside lstlisting, and the escape char % does not
  % work for your specific lstlisting environment, then you should define a different
  % escapechar or define escapeinside in your specific environment. The default
  % escape character is the percent sign.
  % This is subject to change. Let's see how this works in the future.
  % escapeinside={(*@}{@*)},
  % ==== Font ====
  escapechar=\%,
...
% Standard Latin Abbreviations: italicization
\newcommand{\aka}{\mbox{\textit{a.k.a.}}\xspace}
\newcommand{\cf}{\mbox{\textit{cf.}}\xspace}
\newcommand{\deletia}{\textit{\ldots [deletia] \ldots}}
\newcommand{\eg}{\mbox{\textit{e.g.}}\xspace}
\newcommand{\etal}{\mbox{\textit{et al.}}\xspace}
\newcommand{\etc}{\mbox{\textit{etc.}}\xspace}
\newcommand{\ie}{\mbox{\textit{i.e.}}\xspace}
\newcommand{\scil}{\mbox{\textit{sc.}}\xspace} %scilicet: it is permitted to know
\newcommand{\viz}{\mbox{\textit{viz.}}\xspace} %videlicet: it is permitted to see
\newcommand{\vs}{\mbox{\textit{vs.}}\xspace}

% Technical Abbreviations (kept Roman):
...
% Quote and comment.
\newcommand{\qcmt}[2]{\textit{\textcolor{BlindColorTolEight}{\footnotesize CMT: #1}} {\textcolor{blue}{#2}}}

% TODO.
\newcommand{\todo}[1]{\textit{\textcolor{BlindColorTolEight}{\footnotesize TODO: #1}}}
% Quote and todo.
\newcommand{\qtodo}[2]{\textit{\textcolor{BlindColorTolEight}{\footnotesize TODO: #1}} {\textcolor{blue}{#2}}}
...
\newcommand{\lch}[1]{\NamedCommentTemplate{BlindColorTolFive}{Chunhao}{#1}}
...
% Anonymize IDs
\newcounter{myUniqueIdCounter}

\makeatletter
\newcommand{\myGetOrAssignID}[1]{%
  \ifcsname myMap@#1\endcsname%
    % If already defined, return existing ID
    \csname myMap@#1\endcsname%
  \else%
    % If not defined, increment counter and assign
    \stepcounter{myUniqueIdCounter}%
    \expandafter\xdef\csname myMap@#1\endcsname{\themyUniqueIdCounter}%
    \themyUniqueIdCounter%
  \fi%
}
\makeatother
...
% Example usage:
%
% add this line to use anonymized IDs:
%
%    \def\useAnonymizedId{}

\newcommand{\anonymizedId}[1]{\ifx\useAnonymizedId\undefined%
  #1%
\else%
  \myGetOrAssignID{#1}%
\fi}
